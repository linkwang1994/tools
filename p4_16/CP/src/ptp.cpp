...
typedef struct __attribute__((__packed__)) ptp_t
{
  uint8_t dstAddr[6];
  uint8_t srcAddr[6];
  uint16_t type;
...
// ptp Followup
ptp_p ptp_followup_pkt;
uint8_t *upkt;
size_t sz = sizeof(ptp_p);
bf_pkt *bfpkt;

// ptp Request
ptp_p ptp_request_pkt;
uint8_t *dreqpkt;
size_t ptp_sz = sizeof(ptp_p);
bf_pkt *bfptpPkt = NULL;
...
uint32_t ptp_interval = 1000000; //1 sec by default
bool updateEra = false;
bool ptpCalcInProgress = false;
...
void *sendptpRequests(void *args);

bf_status_t initReferenceTsAPI(void);

bf_status_t readReferenceTs(uint32_t *ts_hi, uint32_t *ts_lo, uint8_t switch_id);

bf_status_t reportptpError(uint32_t calc_time_hi, uint32_t calc_time_lo, uint32_t now_elapsed_hi, uint32_t now_elapsed_lo, uint8_t master_switch);
...
bf_status_t writeCalcRefTs(uint32_t calc_time_hi_ptp,
                           uint32_t calc_time_lo_ptp,
...
bf_status_t ptp::setUpBfrt(bf_rt_target_t target, const char *progname)
{
  dev_tgt = target;
  ...
  printf("ptp bfrt Setup!\n");
  return bf_status;
}

bf_status_t ptp::initRegisterAPI(void)
{
  // ts_hi register
...
/* Sets the global 64-bit Reference Time of master.
   ...
   */
bf_status_t ptp::initReferenceTs(void)
{
  struct timespec tsp;
...
void *eraMaintenance(void *args)
{
  ...
  while (1)
  {
    ...
    {
      // Wrap Detected.
      // If there is a ptp calculation in progress, just turn the updateEra flag to true
      // If not, update it here without further delay
      updateEra = true;
      if (!ptpCalcInProgress)
      {
        incrementEra();
      }
    }
...
bf_status_t ptp::createEraThread(void)
{
...
void *sendptpRequests(void *args)
{
  ...
  while (1)
  {
    printf("Sending ptp Packets Out..\n");
    bf_status_t stat = bf_pkt_tx(0, bfptpPkt, tx_ring, (void *)bfptpPkt);
    if (stat != BF_SUCCESS)
    {
...
void ptp::createptpRequestThread(uint32_t interval)
{
  ptp_interval = interval;
  pthread_create(&ptp_thread, NULL, sendptpRequests, NULL);
}

void ptp::waitOnThreads(void)
{
...
bf_status_t initRequestPkt(void)
{
  int i = 0;
  int cookie;
  if (bf_pkt_alloc(0, &bfptpPkt, ptp_sz, BF_DMA_CPU_PKT_TRANSMIT_1) != 0)
  {
    printf("Failed bf_pkt_alloc\n");
  }

  memcpy(ptp_request_pkt.dstAddr, master, 6);
  memcpy(ptp_request_pkt.srcAddr, switch1, 6);
  ptp_request_pkt.type = htons(0x88f7);
  ptp_request_pkt.magic = htons(0x0002);
  ptp_request_pkt.command = ptp_GEN_REQ;
  dreqpkt = (uint8_t *)malloc(ptp_sz);
  memcpy(dreqpkt, &ptp_request_pkt, ptp_sz);
  if (bf_pkt_is_inited(0))
  {
    printf("ptp Request Packet is initialized\n");
  }
  if (bf_pkt_data_copy(bfptpPkt, dreqpkt, ptp_sz) != 0)
  {
...
bf_status_t initFollowupPkt(void)
{
  ...
  memcpy(ptp_followup_pkt.dstAddr, dstAddr, 6); //{0x3c, 0xfd,0xfe, 0xb7, 0xe7, 0xf4}
  memcpy(ptp_followup_pkt.srcAddr, srcAddr, 6); //{0x3c, 0xfd,0xfe, 0xb7, 0xe7, 0xf4}
  ptp_followup_pkt.type = htons(0x88f7);
  ptp_followup_pkt.magic = htons(0x0002);
  ptp_followup_pkt.command = ptp_CAPTURE_COMMAND;

  upkt = (uint8_t *)malloc(sz);
  memcpy(upkt, &ptp_followup_pkt, sz);
...
static bf_status_t ptp::txComplete(bf_dev_id_t device,
...
void ptp::callbackRegister(void)
{
  int tx_ring;
...
bf_status_t ptp::initPackets(void)
{
  callbackRegister();
  initRequestPkt();
  initFollowupPkt();
...
{
  memcpy(ptp_followup_pkt.dstAddr, dstAddr, 6); //{0x3c, 0xfd,0xfe, 0xb7, 0xe7, 0xf4}
  ptp_followup_pkt.reference_ts_hi = htonl(tx_capture_tstamp_lo);
  memcpy(upkt, &ptp_followup_pkt, sz);
  if (bf_pkt_data_copy(bfpkt, upkt, sz) != 0)
  {
    printf("Failed data copy\n");
...
bf_status_t writeCalcRefTs(uint32_t calc_time_hi_ptp,
                           ...
                           uint8_t switch_id)
{
  uint64_t ref_calc_time_hi = calc_time_hi_ptp - now_elapsed_hi;
  uint64_t ref_calc_time_lo;
  if (calc_time_lo_ptp < now_elapsed_lo)
  {
    ref_calc_time_lo = ((uint64_t)calc_time_lo_ptp + (uint64_t)max_32bit) - now_elapsed_lo;
    ref_calc_time_hi -= 1;
  }
  else
  {
    ref_calc_time_lo = calc_time_lo_ptp - now_elapsed_lo;
  }
  writeReferenceTs(ref_calc_time_hi, ref_calc_time_lo, (uint64_t)switch_id);
}

bf_status_t reportptpError(uint32_t calc_time_hi,
                            ...
                            uint8_t master_switch)
{
  ...
  printf("calc_time_hi(ptp)   = %u s, calc_time_lo(ptp)   = %u ns\n", calc_time_hi, calc_time_lo);
  printf("-------------------------------------------------\n");
                                        ...
                                        const void *cookie)
{
  ...
  for (; i < vec.size(); i++)
  {
    ...
    uint32_t calc_time_hi_ptp = s2s_reference_hi[switch_id] + (respTDelay / max_ns) + era;
    uint32_t calc_time_lo_ptp = s2s_reference_lo[switch_id] + (respTDelay % max_ns);

    writeCalcRefTs(calc_time_hi_ptp, calc_time_lo_ptp, now_igts_hi[switch_id], now_igts_lo[switch_id], switch_id);

    ptpCalcInProgress = false;
    updateEra = false;
#if DEBUGLEVEL>=1
    printf("-------------------------------------------------\n");
    printf("                     Switch %d             \n", switch_id);
    ...
    reportptpError(calc_time_hi_ptp, calc_time_lo_ptp, now_igts_hi[switch_id], now_igts_lo[switch_id], 0);
  }
...
bf_status_t ptp::registerDigest(const char *ptp_followup_digest, const char *ptp_reply_digest, const char *ptp_reply_followup_digest)
{ 
  bf_status = bfrtInfo->bfrtLearnFromNameGet(ptp_followup_digest, &bfrtLearnFollowup);
  if (bf_status != BF_SUCCESS)
    return bf_status;
  ...
  bf_status = bfrtInfo->bfrtLearnFromNameGet(ptp_reply_digest, &bfrtLearnReply);
  if (bf_status != BF_SUCCESS)
    return bf_status;
  ...
  bf_status = bfrtInfo->bfrtLearnFromNameGet(ptp_reply_followup_digest, &bfrtLearnReplyFop);
  if (bf_status != BF_SUCCESS)
    return bf_status;
