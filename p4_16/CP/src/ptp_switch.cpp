...
int main(int argc, char **argv) {
	const char * p4progname = "ptp_simple_switch";
	const char * followup_digest = "SwitchIngressDeparser.ptp_ingress_deparser.ptp_followup_digest";
	const char * reply_digest = "SwitchIngressDeparser.ptp_ingress_deparser.ptp_reply_digest";
	const char * reply_followup_digest = "SwitchIngressDeparser.ptp_ingress_deparser.ptp_reply_followup_digest";
	uint32_t ptp_interval = 1000000; // Currently supports upto a request every 2ms, i.e. 500 ptp requests/sec
	bf_rt_target_t dev_tgt;

	// Initialize the device id and pipelines to be used for ptp
    dev_tgt.dev_id = 0;
	...
	sleep(2); // 
	ptp::createptpRequestThread(ptp_interval);

	// Wait on the threads so that ptp runs in the background
	ptp::waitOnThreads();
	return 0;
}
