...
class ModelTime:
    ...
    def __repr__(self):
        return "{}s {:03}ms {:03}us {:03}ns {:03}ps ({}clocks+{:03d}ps)".format(
    ...
    def advance_ps(self, ps=1):
        conn_mgr.advance_model_time(ps)
        conn_mgr.complete_operations()
        self.model_time += ps
        print(self)
        
    def advance_clk(self, clocks=1):
        # Make sure that we advance in whole clocks
        self.advance_ps(clocks * 800 - self.model_time % 800)

    def advance_us(self, us=1):
        # 1 us is 1250 clocks
        self.advance_clk(us * 1250)

    def advance_ms(self, ms=1):
        self.advance_us(ms * 1000)

    def advance_s(self, s=1):
